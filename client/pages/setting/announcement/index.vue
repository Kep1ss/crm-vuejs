                ...
                <table class="table table-striped table-sm vld-parent"
                  ...
                  <tbody>
                    <tr v-for="(item,i) in data" :key="i">
                      <td><input type="checkbox" name="checkboxs[]" :value="item.id" v-model="parameters.form.checkboxs" :disabled="isSuperAdmin"></td>
                      <td>{{ i + 1 }}</td>
                      <td>{{ item.content }}</td>
                      <td v-if="$auth.user.role === 0">{{ item.user ? item.user.username : '-'}}</td>
